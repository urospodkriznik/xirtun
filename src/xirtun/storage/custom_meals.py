...
def add(conn: sqlite3.Connection, name: str, items: list[dict[str, Any]], *, now: datetime | None = None) -> None:
    ...
    t = totals(items)
    conn.execute(
        "INSERT INTO custom_meals (name, items, calories, protein_g, fat_g, carbs_g, sugar_g, fiber_g, sodium_mg, created_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
        ...
        "sugar_g = excluded.sugar_g, fiber_g = excluded.fiber_g, sodium_mg = excluded.sodium_mg",
        (name, json.dumps(items), t["calories"], t["protein_g"], t["fat_g"], t["carbs_g"], t["sugar_g"], t["fiber_g"], t["sodium_mg"], now.isoformat()),
    )
    conn.commit()
